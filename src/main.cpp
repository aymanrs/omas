...
inline float sampleExpDist(float lambda){
    return -log(1-urd(rng))*lambda;
}
struct interaction {
    float t;
    int id1, id2;
...
float asymptote(AgentType type, int n, int threshold, int simTime, float rho, float delta) {
    Agent::idCounter = 0;
    OptimalAgent::time = 0;
    ...
    float lambdaRho = rho ? -1/log(1-rho) : INFINITY;
    float lambdaDelta = rho ? lambdaRho/n : INFINITY;
    float lambdaGamma = n*(n-1)/2;
    float gamma = 1-exp(-1/lambdaGamma);
    OptimalAgent::gamma = gamma;
    float t = 0, nextArrivalTime = rho ? sampleExpDist(lambdaDelta) : INFINITY;
    int actualMax = -1;
    float runAsymtote = 0;
    ...
    for(int i = 0;i < n;i++){
        ...
        if(rho) departures.emplace(
            sampleExpDist(lambdaRho),
            newAgent->_id
        );
        for(auto [id, oldAgent] : agents){
            interactions.emplace(sampleExpDist(lambdaGamma), newAgent->_id, id);
        }
    ...
    while(t < simTime) {
        ...
        if(nextIneractionTime < nextDepartureTime && nextIneractionTime < nextArrivalTime) {
            ...
            TSE += error1*error1 + error2*error2;
            interactions.emplace(t+sampleExpDist(lambdaGamma), id1, id2);
            ...
            nextArrivalTime = t+sampleExpDist(lambdaDelta);
            Agent* newAgent = Agent::makeAgent(type, rng()%1000, threshold);
            if(isOptimal){
                newAgent->interact(newAgent);
            }
            for(auto [id, oldAgent] : agents) {
                interactions.emplace(t+sampleExpDist(lambdaGamma), newAgent->_id, id);
            }
            agents[newAgent->_id] = newAgent;
            departures.emplace(t+sampleExpDist(lambdaRho), newAgent->_id);
...
int main() {
    const int n = 25, T_star = 11, simTime = 1500, nRuns = 50;
    std::vector<float> rhos;
    const float a = 0, b = 0.015, nRhos = 10;
    ...
    for(float& y_i : optimal.y) y_i /= M_SQRT2;
